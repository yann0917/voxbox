...
func newASRCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if engine != "volcengine" && engine != "qianwen" {
	...
	f.StringVar(&engine, "engine", "volcengine", "识别引擎: volcengine（火山引擎）| qianwen（千问，仅录音文件转写）")
	...
	f.StringVar(&language, "language", "", "识别语言（留空自动识别中文/英文/常见方言；可选 zh-CN/en-US/ja-JP/yue-CN 等 25 种；--engine qianwen 时取值 zh/en/ja/ko/de/fr/ru/es/pt/it，留空自动）")
